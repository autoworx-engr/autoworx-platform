...
export const tagModelValidationSchema = z
  .object({
    ...
    name: z.string({ required_error: "Tag name is required", invalid_type_error: "Tag name must be a string" }).min(1, "Tag name is required"),
    textColor: z.string({ required_error: "Text color is required", invalid_type_error: "Text color must be a string" }),
    //   .regex(hexColorRegex, {
    //   message: "Text color must be a valid hex color code (e.g., #000000)",
    // }),
    bgColor: z.string({ required_error: "Background color is required", invalid_type_error: "Background color must be a string" }),
    //   .regex(hexColorRegex, {
    //   message:
    ...
    companyId: z.number({ required_error: "Company ID is required", invalid_type_error: "Company ID must be a number" }).int().positive(),
  })
  .refine(
    (data) => {
