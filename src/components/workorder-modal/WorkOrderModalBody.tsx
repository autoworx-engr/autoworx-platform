...
export default function WorkOrderModalBody({
  ...
  const { data, error, loading } = useServerGet(getWorkOrderData, invoiceId);

  // Move the destructuring inside the rendering section after null checks
  ...
  const { invoice, company, invoiceTechnicians, writePermission } =
