...
export default function FilterByRevenue({
  selectedItem,
  items,
  type,
  modalName,
  ...
  return (
    <div className="relative w-full md:w-auto">
      ...
      {activeModal[modalName as keyof TFilterModalState] && (
        <div
          ref={dropdownRef}
          className="absolute left-0 right-0 z-50 flex max-h-52 w-full flex-col space-y-1 overflow-y-auto rounded-bl-sm rounded-br-sm border border-t-0 border-gray-400 bg-background p-3 md:w-44"
        >
          {items.map((item) => (
            <button
