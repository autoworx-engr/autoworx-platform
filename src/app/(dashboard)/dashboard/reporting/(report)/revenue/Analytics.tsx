...
export default async function Analytics({
    ...
    const lastMonthServiceCategoryRevenue = serviceCategory.reduce(
        (acc, category) => {
            const totalServiceCost = last30DaysInvoice.reduce(
                (acc, invoice) => {
                    ...
                    const serviceProfit = invoiceItems.reduce(
                        (
                            ...
                            const materialSellPrice = item.materials.reduce(
                                (acc, cur) => {
                                    const priceBeforeDiscount =
                                        Number(cur.sell) * Number(cur.quantity);
                                    const discount = Number(cur.discount) || 0;
                                    return (
                                        acc + (priceBeforeDiscount - discount)
                                    );
                                },
                                0
                            );
                            ...
                            const laborDiscount = Number(
                                item.labor?.discount || 0
                            );
                            const laborCost =
                                laborPriceBeforeDiscount - laborDiscount;
        ...
        <div className="rounded-lg border p-6">
            ...
            <div className="min-w-2xl mx-10">
                <RevenueBarChartContainer
                    data={lastMonthServiceCategoryRevenue}
                />
            </div>
        </div>
    );
