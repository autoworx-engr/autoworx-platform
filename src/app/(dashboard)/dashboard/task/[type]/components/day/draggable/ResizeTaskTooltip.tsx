...
export default function ResizeTaskTooltip({
  ...
  const handleResizeTop = (e: React.MouseEvent) => {
    ...
    const handleMouseMove = (moveEvent: MouseEvent) => {
      ...
      const newStartTimeObj = moment(startTimeObj).add(minutesChange, "minutes");
  ...
  return (
    ...
    >
      {/* Top resize handle */}
      <div 
        className="absolute top-0 left-0 h-2 w-full cursor-n-resize rounded-tl-lg rounded-tr-lg bg-transparent hover:bg-gray-200 hover:bg-opacity-50"
        onMouseDown={handleResizeTop}
      >
        {resizing === "top" && (
          <div className="absolute top-0 left-1/2 flex min-w-40 max-w-44 -translate-x-[50%] -translate-y-full items-center justify-center space-x-2 rounded-md bg-stone-200 p-1 text-sm shadow-md">
