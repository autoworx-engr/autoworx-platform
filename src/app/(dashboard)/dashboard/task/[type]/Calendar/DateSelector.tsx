...
import { getWeekInfoFromWeekStr, getWeekStartNumber } from "./utils.DateSelector";
...
function DateSelector({ type, weekStart = 1 }: DateSelectorProps) {
  const { setDate, setWeek, setMonth, date, week, month, setNavigating } = useCalendarStore();
  ...
  const handleWeekSelection = (newWeek: string) => {
    ...
    const firstDayOfWeek = moment()
      ...
      .isoWeek(weekNum)
      .startOf("isoWeeks")
      .format("YYYY-MM-DD");    setDate(firstDayOfWeek);
  ...
  const handleMonthSelection = (newMonth: string) => {
    ...
    const firstDayOfMonth = moment(newMonth)
      .startOf("month")
      .format("YYYY-MM-DD");    setDate(firstDayOfMonth);
