...
export default function Month({
  ...
  const startOfMonth = moment.utc(month).startOf("month").toDate();
  const endOfMonth = moment.utc(month).endOf("month").toDate();
  ...
  function getHolidays(date: Date) {
    // Format the input date in local time zone: 'YYYY-MM-DD'
            ...
            <Tooltip key={i}>
              <TooltipTrigger
                ...
                className={cn(
                  ...
                  isHoliday && "bg-red-50", // Add light red background for holiday cells
                )}                onClick={(event) => {
                  if (
                    cell[0] &&
                    ...
                    // Convert Date object to string to avoid timezone issues
                    const dateString = cell[0] instanceof Date 
                  ...
                  <div
                    className={cn(
                      ...
                      today.getFullYear() === cell[0]?.getFullYear() &&
                        ...
                        : isHoliday
                          ? "border-2 border-red-500 text-red-700 hover:bg-red-200" // Change date display for holidays
                        ...
                        <Tooltip key={i}>
                          <TooltipTrigger asChild>
                            <div
                              ...
                              className="cursor-pointer truncate rounded border-l-4 border-blue-500 bg-blue-50 px-1 py-0.5 text-xs text-slate-700 hover:bg-blue-100 lg:block xl:text-sm"
                            >
                              {appointment.title}
                            </div>
                          </TooltipTrigger>
                      ...
                      .map((task: CalendarTask, i: number) => (
                        <Tooltip key={i}>
                          <TooltipTrigger asChild>
                            <div
                              onClick={() => handleRedirectToDay(cell[0])}
                              className="cursor-pointer truncate rounded px-1 py-0.5 text-xs text-white lg:block lg:text-sm"
                              style={{
                            ...
                            (appointment: CalendarAppointment, i: number) => {
                              ...
                              return (
                                <Tooltip key={i}>
                                  {moreLeft > 0 && (
                                    <button
                                      className="text-left text-xs font-normal text-slate-500"                                      onClick={(event) => {
                                        if (
                                          event.target instanceof Node &&
              ...
              <TooltipPortal>
                ...
                {(cell[1]?.length || cell[2]?.length || cell[3]?.length) && (
                  <TooltipContent>
                    <div className="max-h-[350px] w-[350px] overflow-y-scroll">
                      ...
                      {cell[3]?.length > 0 && (
                        <>
                          ...
                          <div className="mb-3 rounded border-l-4 border-red-500 bg-red-50 p-3">
                            {cell[3].map((holiday: Holiday, i: number) => (
                              <div
                                key={i}
                                className="flex items-center justify-between"
                              >
                                <p className="text-red-700">Holiday</p>
                              </div>
                            ))}
                          </div>
                        </>
