...
export function NewAppointment({
  ...
  useEffect(() => {
    ...
    if (data?.date) {
      if (typeof data.date === "string" && data.date.includes("-")) {
        ...
        try {
          const dateObj = moment(data.date).toDate();
  ...
  useEffect(() => {
    ...
    const filteredTechnicians = employees.filter((employee) => {
      return (
        employee.employeeType === "Technician" &&
        !assignedUsers.find((user) => user.id === employee.id)
      );
  ...
  return (
    <div className="newAppointment">
      <Dialog open={popup === "ADD_TASK"} onOpenChange={setOpen}>
        ...
        <DialogContent
          className="grid max-h-full max-w-5xl grid-rows-[auto,1fr,auto] sm:max-w-[60vw] max-sm:[&>button]:right-4 max-sm:[&>button]:top-3 [&>button>svg]:w-6 [&>button>svg]:h-6"
          form
        >
          {/* Heading */}
          ...
          <DialogFooter className="justify-end">
            ...
            <Submit
              className="mb-2 rounded-md border bg-[#6571FF] px-4 py-1 text-white md:mb-0"
