...
import { Sparkles,  User, Lightbulb, Volume2, MessageSquare, Brain, Loader2 } from "lucide-react";
import { Button } from "@/components/ui/button";
import { useAiPersonality, useSaveAiPersonality } from "@/hooks/ai-train/useAiPersonality";
import { RadioGroup, RadioGroupItem } from "@/components/ui/radio-group";

import { Textarea } from "@/components/ui/textarea";
import { PERSONALITY_TYPES, SLIDER_CONFIGS, VOICE_TONE_OPTIONS } from "../constants/constants";
...
useEffect(() => {
  ...
  const handleSliderChange = useCallback((field: 'warmth' | 'humor' | 'energy', value: number[]) => {
    setPersonality(prev => ({ ...prev, [field]: value[0] }));
  }, []);

const handleInputChange = useCallback((field: 'assistantName' | 'openingMessage' | 'systemPrompt', value: string) => {
    setPersonality(prev => ({ ...prev, [field]: value }));
  }, []);

  const handleSwitchChange = useCallback((
    field: 'useContractions' | 'useCasualLanguage' | 'matchCustomerTone' | 'useEmojis', 
    value: boolean
  ) => {
    setPersonality(prev => ({ ...prev, [field]: value }));
  }, []);
  ...
  const handlePersonalityTypeChange = useCallback((value: string) => {
    setPersonality(prev => ({ ...prev, personalType: value as PersonalityType }));
  }, []);
