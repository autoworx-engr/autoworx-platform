...
import {  DaysOfWeek, Frequency, InventoryActions, InventoryConditions } from "./constants";
  ...
  const loadData = async () => {
    if (isEdit && id) {
      setFormData({
        ...
        day: data?.data.day,
        teamMemberUserIds: data?.data?.teamMembers?.map((item: any) => item.userId)
      });
    } else {
      setFormData({
        companyId: null,
...
errorToast("Something went wrong!");
  ...
  return (
   <>
   {loading || isLoading || isFetching  ?(<div className="flex h-[800px] w-full animate-pulse items-center justify-center rounded-md bg-gray-200 p-4 shadow-sm md:p-6">
             <Spin />
           </div>) : 
     (<div>
      <div className="h-[600px] rounded-md border bg-white p-4 shadow-sm md:p-6">
        <Paper elevation={0} className="mx-auto max-w-lg rounded-lg">
          <form onSubmit={handleSubmit}>
            ...
            {formData.frequency === "WEEKLY" && <Selector
              name="day"
              label="Day"
              options={DaysOfWeek}
              value={formData.day}
