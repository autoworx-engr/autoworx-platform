...
const DesignPicker = ({ designs, selected, onSelect, shopName }: Props) => {
  ...
  return (
    <div className="space-y-6">
      <div>
        <h3 className="text-lg font-semibold tracking-tight mb-1" style={{ fontFamily: "'Space Grotesk', sans-serif" }}>Choose a Design</h3>
        <p className="text-sm text-muted-foreground">Select a gift card style</p>
      </div>
      ...
      {selectedDesign && (
        <div className="relative rounded-2xl overflow-hidden shadow-lg aspect-[16/9] max-w-md mx-auto border border-border">
          ...
          <div className="absolute bottom-4 left-4 right-4">
            <p className="text-white text-xs font-medium opacity-80">{shopName}</p>
            <p className="text-white text-lg font-bold tracking-tight" style={{ fontFamily: "'Space Grotesk', sans-serif" }}>Gift Card</p>
          </div>
        </div>
      )}
        ...
        {enabledDesigns.map(d => (
          <button
            ...
            className={cn(
              ...
              selected === d.id ? "border-primary ring-2 ring-primary/30" : "border-border hover:border-muted-foreground/40"
            )}
          >
