    ...
    <>
      <Dialog open={open} onOpenChange={setOpen}>
        <DialogTrigger asChild>
          <button className="flex items-center gap-2 rounded border bg-background px-4 py-1 text-[#6571FF]">
            <span>Refund</span>
            <CiSettings />
          </button>
        </DialogTrigger>
              ...
              <div className="flex items-center gap-4">
                {paymentMethods.map((method, idx) => (
                  <button
                    ...
                    className={`flex items-center gap-x-2 rounded-md border border-[#6571ff] px-4 py-1 capitalize ${checkedMethod === method.method ? "bg-[#6571ff] text-white" : "bg-background text-[#6571ff]"}`}
                  >
                    {method.icon}
