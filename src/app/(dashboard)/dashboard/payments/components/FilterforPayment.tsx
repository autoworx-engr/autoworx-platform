...
const FilterforPayment = () => {
  ...
  return (
    <div className="relative" ref={ref}>
      ...
      <button
        className={cn(
          "flex h-10 items-center justify-center rounded-xl px-4 text-sm font-medium transition-all duration-300 min-w-[120px] focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-[#6571ff]",
          "active:shadow-lg",
          paymentMethod !== "All"
      ...
      {showFilter && (
        <div className="absolute -right-16 z-40 mt-2 md:right-auto [@media(min-width:375px)]:-right-4 [@media(min-width:425px)]:-right-[4.5rem] animate-in fade-in zoom-in-95 duration-200 origin-top-right">
          <div className="mt-2 w-72 rounded-2xl border border-slate-200 dark:border-slate-700 bg-white/90 dark:bg-slate-900/90 backdrop-blur-xl p-5 shadow-2xl shadow-slate-200/50 dark:shadow-slate-900/50 lg:w-[400px] [@media(min-width:375px)]:w-80 [@media(min-width:425px)]:w-96">
            <div className="mb-6">
              <div className="font-Inter mb-3 text-sm font-semibold text-slate-700 dark:text-slate-200">Payment Method</div>
            ...
            <div className="mb-6">
              <div className="font-Inter mb-3 text-sm font-semibold text-slate-700 dark:text-slate-200">Amount</div>
              <div className="flex items-center">
                <span className="mr-4 text-sm font-medium text-slate-600 dark:text-slate-300 w-fit text-center">${amount[0]}</span>
                <div className="flex-1">
                  <SliderRange value={amount} onChange={handleSliderChange} />
                </div>
                <span className="ml-4 text-sm font-medium text-slate-600 dark:text-slate-300 w-fit text-center">${amount[1]}</span>
              </div>
            </div>
            <div className="mb-6">
                ...
                {["All", "Paid", "Unpaid"].map((statusOption) => (
                  ...
                  >
                    <input
                      ...
                      className="accent-[#6571ff] h-4 w-4 transition-all"
                    />
                    <span className="text-sm text-slate-600 dark:text-slate-300 group-hover:text-slate-900 dark:group-hover:text-white transition-colors">{statusOption}</span>
                  </label>
                ))}
              </div>
            </div>
