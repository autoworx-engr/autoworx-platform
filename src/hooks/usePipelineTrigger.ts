...
import { Appointment, ConditionType, Task } from "@prisma/client";
import { useQueryClient } from "@tanstack/react-query";

type UpdatePipelineAutomationTriggerPayload = {
...
type ActionMap = {
  UPDATE_PIPELINE_AUTOMATION_TRIGGER: UpdatePipelineAutomationTriggerPayload;
...
export default function usePipelineTrigger() {
  ...
  const handleAutomationTrigger = async ({
    ...
    leadId,
    columnId,
  }: UpdatePipelineAutomationTriggerPayload) => {
    try {
      const updatedLead = await updatePipelineAutomationTrigger({
        condition,
        companyId,
        leadId,
        columnId,
  ...
  function dispatch<T extends TActions>(action: T, payload: ActionMap[T]) {
    switch (action) {
      case "UPDATE_PIPELINE_AUTOMATION_TRIGGER":
        handleAutomationTrigger(
          payload as UpdatePipelineAutomationTriggerPayload,
        );
        break;
