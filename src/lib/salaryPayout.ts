...
import { getSalaryHistoryForPeriod, getActiveSalary } from "@/lib/salaryHistoryManager";

/**
 * Calculate salary-based previous month earnings
 */
export async function calculateSalaryPreviousMonthEarnings(targetUserId?: number) {
  const { companyId, userId } = await getEssentials(targetUserId);
 ...
 */
export async function calculateSalaryCurrentMonthEarnings(targetUserId?: number) {
  const { companyId, userId } = await getEssentials(targetUserId);
  const { timezone } = await getCompanyTimezone();
  ...
  if (activeSalary.salaryType === "BI_WEEKLY") {
    ...
    const daysSinceStart = Math.floor((now.getTime() - salaryStartDate.getTime()) / (24 * 60 * 60 * 1000));
    const currentPeriodNumber = Math.floor(daysSinceStart / 14);
    const currentPeriodStart = new Date(salaryStartDate.getTime() + (currentPeriodNumber * 14 * 24 * 60 * 60 * 1000));
 ...
 */
export async function calculateSalary2ndPreviousMonthEarnings(targetUserId?: number) {
  const { companyId, userId } = await getEssentials(targetUserId);
  const { timezone } = await getCompanyTimezone();
  
  // Calculate 2nd previous month dates
  const now = new Date();
  const secondPreviousMonth = new Date(now.getFullYear(), now.getMonth() - 2, 1);
  const secondPreviousMonthEnd = new Date(now.getFullYear(), now.getMonth() - 1, 0);
 ...
 */
export async function calculateSalaryTotalEarnings(targetUserId?: number) {
  const { companyId, userId } = await getEssentials(targetUserId);
  const { timezone } = await getCompanyTimezone();
...
async function calculateSalaryForPeriodWithHistory(
  ...
  const salaryPeriods = await getSalaryHistoryForPeriod(userId, periodStart, periodEnd);
  ...
  for (const salaryPeriod of salaryPeriods) {
    ...
    const effectiveStart = new Date(Math.max(periodStart.getTime(), salaryPeriod.startDate.getTime()));
    const effectiveEnd = new Date(Math.min(
      periodEnd.getTime(), 
      salaryPeriod.endDate ? salaryPeriod.endDate.getTime() : new Date().getTime()
    ));
...
async function calculateSalaryForPeriod(
  ...
  const effectiveStart = periodStart < salaryStartDate ? salaryStartDate : periodStart;

  switch (salaryType) {
    case "HOURLY":
      return await calculateHourlyForPeriod(userId, Number(salaryAmount), effectiveStart, periodEnd);
    
    case "WEEKLY":
      return calculateWeeklyForPeriod(Number(salaryAmount), salaryStartDate, effectiveStart, periodEnd);
    
    case "BI_WEEKLY":
      return calculateBiWeeklyForPeriod(Number(salaryAmount), salaryStartDate, effectiveStart, periodEnd);
    
    case "MONTHLY":
      return calculateMonthlyForPeriod(Number(salaryAmount), salaryStartDate, effectiveStart, periodEnd);
...
function calculateWeeklyForPeriod(
  ...
  while (currentWeekStart <= new Date().getTime()) {
    ...
    const endsInPeriod = hasCompleted && 
...
function calculateMonthlyForPeriod(
  ...
  while (true) {
    ...
    const overlapsTarget = monthEnd >= effectiveStart && monthStart <= periodEnd;
 ...
 */
async function getEssentials(targetUserId?: number) {
