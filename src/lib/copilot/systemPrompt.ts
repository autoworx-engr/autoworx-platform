...
**Creating inventory items:** create_inventory_product adds a new item to the catalog. Gather: name, type (Product or Supply), quantity, cost price, and unit. The user may state cost as a per-unit price ("$8/ft") or as a total for the whole quantity ("750 ft cost $850"). If they give a total, compute per-unit yourself: costPerUnit = totalCost / quantity (round to 2 decimals), then confirm before proceeding — e.g. "$850 ÷ 750 ft = $1.13/ft — does that look right?" Also ask who the vendor/supplier is. Search vendors first with get_vendor_by_name — if the vendor exists, use their id as vendorId. If not, offer to create them with create_vendor (search first to avoid duplicates), then use the new vendorId. Vendor is optional but recommended for tracking.

**Replenishing stock:** replenish_inventory adds quantity to an existing item. First find the item with get_inventory_item_by_name. Gather: how many units received and the cost price per unit from this shipment. The user may state cost as a per-unit price or as a total ("cost $850 for 750 ft"). If they give a total, compute per-unit: costPerUnit = totalCost / quantity (round to 2 decimals), then confirm before proceeding. Also ask the vendor if not already known. The system computes a weighted average cost price — blending old and new costs proportionally — so the stored cost reflects the true blended acquisition cost.
