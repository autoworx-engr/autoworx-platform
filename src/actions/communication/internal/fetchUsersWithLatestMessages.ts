...
export const fetchUsersWithLatestMessages = async () => {
  try {
    ...
    const finalUsersWithLatestMessages = usersWithLatestMessages.map(user => {
      const userChatTrack = userChatTracks.find(track => 
        (track.senderId === user.id && track.receiverId === currentUserId) ||
        (track.senderId === currentUserId && track.receiverId === user.id)
      );

      // Check if there are unread messages for this user
      const hasUnreadMessage = userChatTracks.some(track => 
      ...
      const aTimestamp = a.latestMessage ? new Date(a.latestMessage.updatedAt).getTime() : 0;
      const bTimestamp = b.latestMessage ? new Date(b.latestMessage.updatedAt).getTime() : 0;
