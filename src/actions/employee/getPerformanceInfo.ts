...
export async function getPerformanceInfo(id: number) {
  ...
  //   }, {}),
  // ).map(([categoryName, count]) => ({ categoryName, count }));
  // Calculate the total number of jobs assigned
  const totalJobsAssigned = user.Technician.length;

  // Calculate the total number of jobs completed on time
  const totalJobsCompletedOnTime = user.Technician.filter((job) => {
    const dateClosed = moment(job.dateClosed);
    const due = moment(job.due);
    return (
      job.status === "Complete" &&
      (dateClosed.isSameOrBefore(due) || (!job?.due && job.dateClosed))
    );
  }).length;

  // Calculate the total number of jobs completed late
  const totalJobsCompletedLate = user.Technician.filter((job) => {
    if (!job?.due || !job.dateClosed) return false;
    const dateClosed = moment(job.dateClosed);
    const due = moment(job.due);
    return job.status === "Complete" && dateClosed.isAfter(due);
  }).length;
  ...
  return {
    ...
    totalJobs: totalJobsAssigned,
    totalJobsCompletedOnTime: totalJobsCompletedOnTime,
    totalJobsCompletedLate: totalJobsCompletedLate,
  };
}
