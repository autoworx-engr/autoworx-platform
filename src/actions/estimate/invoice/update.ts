...
export async function updateInvoice(
  ...
  try {
    ...
    const updatedInvoice = await db.$transaction(async (db) => {
      ...
      console.log("🚀 ~ updatedInvoice ~ materials:", materials);
      ...
      await db.invoiceInspection.deleteMany({
        where: {
          invoiceId: data.id,
        },
      });
      ...
      await Promise.all(
        data.items?.map(async (item) => {
          ...
          if (labor?.id) {
            ...
            if (existingLabor && existingLabor.cannedLabor === false) {
          ...
          getTechnicianForThisService.map(async (technician) => {
            ...
            await addVehicleParts(technician.vehicleParts, newTechnician.id);
          }),
          ...
          const materialsCreatePromise =
            materials?.length > 0
              ? materials?.map(async (material) => {
                  ...
                  // if (
                  //   Number(material?.cost || 0) > Number(material?.sell || 0)
                  // ) {
                  //   throw new Error(
    ...
    latestInvoice.invoiceItems = latestInvoice?.invoiceItems.map(
      (item: any) => {
