...
export function encodeCompanyId(companyId: string): string {
  ...
  const encoded = Buffer.from(`awx_${companyId}_${Date.now().toString(36)}`).toString('base64');
  return encoded.replace(/[+=]/g, '').substring(0, 16); // Remove padding and truncate
}

export function decodeCompanyId(encodedId: string): string | null {
  try {
    // Pad the encoded string if necessary
    const padded = encodedId + '=='.substring(0, (4 - encodedId.length % 4) % 4);
